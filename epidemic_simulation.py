...
def get_epidemic_infection_times(inf_nodes, inf_prob, movement_info):
  ...
  all_the_nodes = set(movement_info['Source']).union(set(movement_info['Destination']))
...
def get_epidemic_infection_times_np_array(inf_nodes, inf_prob, movement_info_np_array, immunized_nodes_list):
  ...
  for mov in sorted(mov_inf, key=lambda x: x[3]):
    ...
    if inf_time[mov[0]] is None or inf_time[mov[0]] >= mov[2]:
      ...
      if len(all_the_nodes.difference(infected_nodes)) == 0:
        t = time.gmtime(mov[3])
        print('All the {} nodes were infected when the last healthy node ({}) was infected on {}.{}.{} at {}:{}'.format(len(infected_nodes), mov[1], t[2], t[1], t[0], t[3], t[4]))
        break

      if i == 0:
        t = time.gmtime(mov[3])
        print('First infection to node {} on {}.{}.{} at {}:{}'.format(mov[1], t[2], t[1], t[0], t[3], t[4]))
        i += 1
